...
def process_predictions(
    ...
    for hypo in hypos[: min(len(hypos), args.nbest)]:
        ...
        print('tgt_words', tgt_words)
        
        hyp_words = process_str(hyp_words)
        tgt_words = process_str(tgt_words)

        print('hyp_words:', hyp_words)
        print('tgt_words', tgt_words)
        return editdistance.eval(hyp_words, tgt_words), len(tgt_words)

def process_str(word:str):
    result = []

    pt0, pt1 = 0, 0
    
    while pt1 < len(word):
        if word[pt1] != '<' and word[pt1] != '>':
            # result.append(word[pt1])
            pt1 += 1
        elif word[pt1] == '<':
            result += list(word[pt0: pt1])
            pt0 = pt1
            pt1 += 1
        elif word[pt1] == '>':
            result.append(word[pt0: pt1+1])
            pt1 += 1
            pt0 = pt1

    if pt1 == len(word) and pt0 != pt1:
        result+=list(word[pt0: pt1+1])
...
# python3 examples/speech_recognition/infer.py /content/weak_supervision/data --task audio_pretraining \
#   --nbest 1 --path /content/outputs/2021-04-18/01-43-30/checkpoints/checkpoint_best.pt --gen-subset dev_other --results-path /content \
#   --criterion ctc --labels ltr --max-tokens 4000000 --w2l-decoder viterbi \
#   --post-process letter --beam 1

# for max decoding beam=1