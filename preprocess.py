            ...
            while total < max_duration:
                ...
                for i, data in df.iterrows():
                    ...
                    if total >= max_duration:
                        ...
                        print(
                            f'finished sampling for {total/3600} hrs, csv file saved in {df_save_path} and audio saved in {sav_path}')
...
def convert_text_to_index(df,
        ...
        for i, data in df.iterrows():
            ...
            if len(sentence) == 0:
                try:
                    os.remove(wav_path)
                    print(
                        f'deleted <{wav_name}> from dir. Length of sentence is < 1')
                except:
                    print(
                        f'You need to delete <{wav_name}> from dir. Length of sentence is < 1')
                    continue
                continue
    ...
    print(
        f'Total duration of file added from {audio_path} is {total_sec/3600}hrs')
...
def resample_audio_for_training(df_main, src_path,
                   ...
                   shuffle=True,
                   seed=0):
    """Splits data into two genders, resample data and 
    selects only data greater than {max_duration}
    """
    make_dirs(dest_path)
    ...
    for gender in df_dict:
      ...
      for i, data in df.iterrows():
        if duration_len[0] < data.duration < duration_len[1]:
          wav_file = data.path
          src = os.path.join(src_path, wav_file)
    ...
    print(
        f'Resampling for {temp_df.duration.sum()} secs finished in {time.time() - t0} seconds')

    return temp_df


def copy_data(df, src_path, dst_path, max_duration=5*3600, resample=False, shuffle=True):
    """Randomly samples "max_duration" seconds from dataframe and move files to dest path.
    Does not resample audio. Shuffles dataframe before resampling if enabled.
    """
    make_dirs(dst_path)
    if shuffle:
